...
class Campaigns extends BaseController
{
    ...
    public function create() { $data=$this->payload(); $m=new CampaignModel(); if(!$m->insert($data)) return redirect()->back()->withInput()->with('error',implode(' ',$m->errors())); return redirect()->to('/app/campaigns')->with('success','Campaign saved as '.$data['status'].'.'); }
